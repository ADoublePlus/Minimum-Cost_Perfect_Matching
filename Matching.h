...
class Matching
{
    public:
        ...
        /*
            ...
            Receives the vector whose position 'i' has the cost of the edge with index i.
            If the graph doesn't have a perfect matching, a const char * exception will be raised.
            Returns a pair.
    ...
    private:
        ...
        // Use a heuristic algorithm to find the maximum matching of the graph
        void Heuristic();

        // Modifies the costs of the graph so all edges have positive costs
        void PositiveCosts();
       ...
       vector<int> outer; // outer[v] gives the index of the outer-most blossom that contains v, outer[v] = v if v is not contained in any blossom
       ...
       vector<list<int>> shallow; // shallow[v] is a list of the vertices immediately contained inside v, shallow[v] is empty by default
       vector<int> tip; // tip[v] is the tip of blossom v 
       vector<bool> active; // True if a blossom is being used

       vector<int> type; // Even, odd or neither (2, 1, 0)
